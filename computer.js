...
let checkWin = false;
 ...
 let currentPlayer=1 
     ...
     function computerPlay(){
        ...
        let num = 40 - (Math.floor(Math.random() * 40));
      do {
         num = 40 - (Math.floor(Math.random() * 40));
      }while( checkWin == false &&!playerCill[num+7].classList.contains("taken") || playerCill[num].classList.contains("taken"));

        if(currentPlayer===2 && checkWin == false){ 
            currentPlayer=1 ;
            player.innerHTML= "player 1" ;
           
            playerCill[num].className="player-two  taken" ;
 ...
 function PlaySound2(){
     ...
     function winning(){
         ...
         if(Cell.every(win=>gameCells[win].classList.contains("player-one"))){
             ...
             $("#winner").html("computer ").css("color" , "blue");
             PlaySound2();
             checkWin= true;
         }
         }
